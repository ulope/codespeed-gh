...
def create_report_if_enough_data(rev, exe, e, base_branch=None):
    """Triggers Report creation when there are enough results"""
    last_revs = Revision.objects.filter(
        branch=rev.branch if not base_branch else base_branch
    ).order_by('-date')[:2]
    if len(last_revs) > 1:
        ...
        if len(current_results) >= len(last_results):
            ...
            kwargs = dict(executable=exe, environment=e, revision=rev)
            if base_branch:
                kwargs['base_branch'] = base_branch
            report, created = Report.objects.get_or_create(**kwargs)
            report.full_clean()
            report.save()
            ...
            logger.warn("Insufficient new results avialable to create report")
