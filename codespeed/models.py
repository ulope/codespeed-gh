...
class Revision(models.Model):
    ...
    def __str__(self):
        ...
        string = " - ".join(filter(None, (date, self.commitid[:10], self.tag)))
...
class Report(models.Model):
    ...
    base_branch = models.ForeignKey(Branch, blank=True, null=True, related_name='fork_reports')
    _tablecache = models.TextField(blank=True)
    average_change = models.FloatField(blank=True, null=True)

    def __str__(self):
        if self.base_branch:
            return u"Report based on %s for %s" % (self.base_branch, self.revision)
        else:
            return u"Report for %s" % self.revision

    class Meta:
        unique_together = ("revision", "executable", "environment", "base_branch")
    ...
    def get_changes_table(self, trend_depth=10, force_save=False):
        ...
        # If a base_branch is defined use it, otherwise use the same as our own
        # Get latest revisions for this branch (which also sets the project)
        try:
            lastrevisions = Revision.objects.filter(
                branch=self.base_branch if self.base_branch else self.revision.branch
            ).filter(
